      ...
      <div class="cart-body">
        <ul class="cart-list" v-for="(cart,index) in cartInfoList" :key="cart.id">
          ...
          <li class="cart-list-con7">
            <a class="sindelet" href="#" @click="deleteCartById(cart)">删除</a>
            <br>
            <a href="#none">移到收藏</a>
          </li>
        </ul>
...
export default {
  ...
  methods: {
    ...
    async updateChecked(cart, event) {
      ...
      try {
        ...
        await this.$store.dispatch('upateCheckedById', { skuId, isChecked })
        this.getData()
      } catch (error) {
        alert(error.message)
      }
  ...
  computed: {
    ...
    //判断底部复选框是否勾选
    isAllCheck() {
      // every()方法的定义与用法:every()方法用于检测数组中的所有元素是否都满足指定条件(该条件为一个函数)
      //只要有一个不是1===>假
